...
###### Code
{% highlight python %}
'''Extract latest stories from Times of India, Guwahati Page'''
...
{% highlight python %}
'''
In browser do (right click) viewsource or inspect element and find out the location of the latest stories. Here it is inside html element 'ul' of class 'list5 clearfix'. Refer to Beautiful soup documentation (link above) for more details.
'''
newslist = soup.find('ul', attrs={'class':'list5 clearfix'})
for news in newslist:
